...
function process_form($form) {
	if ($_SERVER['REQUEST_METHOD'] != 'POST')
		die(get_form_error_response('Nieznana metoda żądania serwera'));

	if (formthrottle_too_many_submissions($_SERVER['REMOTE_ADDR']))
		die(get_form_error_response('Z tego IP przesłano ostatnio zbyt wiele zgłoszeń'));
	...
	switch($properties['type']) {
		...
		case 'recaptcha':
			if (!array_key_exists('recaptcha', $form) || !array_key_exists('private_key', $form['recaptcha']) || empty($form['recaptcha']['private_key']))
				die(get_form_error_response('Nieprawidłowy klucz prywatny reCAPTCHA.'));
		...
		default:
			die(get_form_error_response(sprintf('Nieznany typ pola „%s”.', $properties['type'])));
	}
}
...
function email_form_submission($form) {
	...
	if(!$sent)
		die(get_form_error_response('Nie udało się wysłać wiadomości e-mail'));
...
function get_email_body($subject, $heading, $fields) {
	...
	$message .= '</table>';
	$message .= '<br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Formularz wysłany z witryny: ' . encode_for_form($_SERVER['SERVER_NAME']) . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Adres IP gościa: ' . encode_for_form($_SERVER['REMOTE_ADDR']) . '</div>';
	$message .= '</body></html>';
...
function get_form_field_value($field, $properties) {
	...
	switch($properties['type']) {
		case 'checkbox':
			return (($value == '1' || $value == 'true') ? 'Zaznaczone' : 'Niezaznaczone');
